...
ジョブ実行に当たっての注意点ですが、例えばpackage.ymlで任意のパッケージをインストールしたい場合、パッケージインストールはdbtのバージョン指定がされているケースが多く、dbtバージョンを変えるには以下の記述が必要になります(明示的な記載がなく軽くハマりました)。Fivetranでは[dbtのサポートバージョン](https://fivetran.com/docs/changelog)が指定されており、サポート対象外のバージョンを利用する場合は`--no-version-check`で一応のerror回避はできそうです。
